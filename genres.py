...
def add_data(genre, c, conn):
     c.execute('INSERT INTO genres VALUES ' + '(' + "'" + genre + "'" ')')
     print('INSERT INTO genres VALUES ' + '(' + "'" + genre + "'" ')')
    #  conn.commit()
...
def update_data(task2, task1, c, conn):
     stri = 'UPDATE genres SET ' + task1  + ' WHERE ' + task2
     print(stri)
     c.execute(stri)
    #  conn.commit()

def delete_data(task1, c, conn):
     c.execute('DELETE FROM genres WHERE ' + task1)
     print('DELETE FROM genres WHERE ' + task1)
    #  conn.commit()

def read_data(c, conn):
     with st.expander("View Genres"):
        #   if st.button('reload'):
        #       result = view_all_data(c)
          result = view_all_data(c)
          clean_df = pd.DataFrame(result, columns=['genre'])
     ...
     with tab1:
        ...
        if st.button('Insert'):
            add_data(genre, c, conn)
            st.success('Inserted')
     
     with tab2:
        st.subheader('Update Data')
        list_of_tasks = set({'genre'})
        selected_task = st.selectbox("Column Name", list_of_tasks)
        task1 = selected_task + " = " + "'" + st.text_input(selected_task, key='task1') + "'"
        if len(list_of_tasks-{selected_task})<1:
            update_task = st.selectbox("Column to update", list_of_tasks)
            task2 = selected_task + " = " + "'" + st.text_input(selected_task, key='task2') + "'"
        else:
     ...
     with tab4:
         ...
         if st.button("Execute"):
             with st.expander('View Table'):
                ...
                clean_df = pd.DataFrame(data, columns=['genre'])
                st.dataframe(clean_df)